...
const App: React.FC = () => {
  ...
  const handleLogin = (loggedInUser: User) => {
    // Verifica se é a primeira vez que este usuário loga neste dispositivo
    const isNewLead = !localStorage.getItem('mettafort_user');
    
    setUser(loggedInUser);
    localStorage.setItem('mettafort_user', JSON.stringify(loggedInUser));

    // Se for um novo usuário, registra o lead de forma oculta
    if (isNewLead) {
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex justify-around p-3 z-50">
        {navItems.map(item => (
            <button
                key={item.page}
                onClick={() => setCurrentPage(item.page)}
                className={`p-2 rounded-xl flex flex-col items-center ${currentPage === item.page ? 'text-teal-600 bg-teal-50' : 'text-gray-400'}`}
            >
